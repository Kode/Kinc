// This header file's purpose is to kill Windows A functions, for the purpose of preventing localization
// mistakes in application code. Windows A functions are not UTF-8-aware and cannot be used in any practical
// way in properly engineered localization-compatible code. What you want to do with this header is set your
// build to either auto-include this for builds (VC++ supports this) or otherwise make sure it is #included 
// in every compilation unit. If code attempts to use an unsafe function then an error will be generated.
