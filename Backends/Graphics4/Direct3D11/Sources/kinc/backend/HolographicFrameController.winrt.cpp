...
void HolographicFrameController::setupSpatialLocator() {
	...
	m_positionalTrackingDeactivatingToken = m_spatialLocator->PositionalTrackingDeactivating +=
	...
	m_positionalTrackingLocatabilityChangedToken = m_spatialLocator->LocatabilityChanged +=
...
void HolographicFrameController::beginRender(int eye) {
	...
	auto m_d3dViewport =
	    CD3D11_VIEWPORT(m_currentCamPose->Viewport.Left, m_currentCamPose->Viewport.Top, m_currentCamPose->Viewport.Width, m_currentCamPose->Viewport.Height);
...
Microsoft::WRL::ComPtr<IDXGIAdapter3> HolographicFrameController::getCompatibleDxgiAdapter() {
	...
	if ((id.HighPart != 0) && (id.LowPart != 0)) {
		...
		affirm(CreateDXGIFactory2(createFlags, IID_PPV_ARGS(&dxgiFactory)));
		ComPtr<IDXGIFactory4> dxgiFactory4;
		affirm(dxgiFactory.As(&dxgiFactory4));

		// Retrieve the adapter specified by the holographic space.
		affirm(dxgiFactory4->EnumAdapterByLuid(id, IID_PPV_ARGS(&theDxgiAdapter)));
	}
	return theDxgiAdapter;
}

void HolographicFrameController::onPositionalTrackingDeactivating(SpatialLocator ^ sender, SpatialLocatorPositionalTrackingDeactivatingEventArgs ^ args) {
	// Without positional tracking, spatial meshes will not be locatable.
	args->Canceled = true;
}

void HolographicFrameController::onPositionalTrackingLocatabilityChanged(SpatialLocator ^ sender, Object ^ args) {
...
void HolographicFrameController::onCameraAdded(HolographicSpace ^ sender, HolographicSpaceCameraAddedEventArgs ^ args) {
...
void HolographicFrameController::onCameraRemoved(HolographicSpace ^ sender, HolographicSpaceCameraRemovedEventArgs ^ args) {
