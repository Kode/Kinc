...
VideoFrameProcessor::VideoFrameProcessor(Platform::Agile<MediaCapture> mediaCapture, MediaFrameReader ^ reader, MediaFrameSource ^ source)
    : m_mediaCapture(std::move(mediaCapture)), m_mediaFrameReader(std::move(reader)), m_mediaFrameSource(std::move(source)) {
	// Listen for new frames, so we know when to update our m_latestFrame
	m_mediaFrameReader->FrameArrived +=
	    ref new TypedEventHandler<MediaFrameReader ^, MediaFrameArrivedEventArgs ^>(std::bind(&VideoFrameProcessor::onFrameArrived, this, _1, _2));
}
		...
		return create_task(mediaCapture->InitializeAsync(settings)).then([=] {
			...
			return create_task(mediaCapture->CreateFrameReaderAsync(selectedSource, MediaEncodingSubtypes::Bgra8)) // TODO mawe: format works on hololens?
			    .then([=](MediaFrameReader ^ reader) {
				    return create_task(reader->StartAsync()).then([=](MediaFrameReaderStartStatus status) {
...
CameraImage *VideoFrameProcessor::getCurrentCameraImage(SpatialCoordinateSystem ^ worldCoordSystem) {
	...
	CameraImage *cameraImage =
	    new CameraImage(format->Width, format->Height, nullptr, WindowsNumericsToKoreMat(cameraToWorld->Value), projectionMat, focalLength);
