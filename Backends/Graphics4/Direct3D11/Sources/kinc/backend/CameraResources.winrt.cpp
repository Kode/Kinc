...
void DX::CameraResources::createResourcesForBackBuffer(DX::DeviceResources *pDeviceResources, HolographicCameraRenderingParameters ^ cameraParameters) {
	...
	if (m_d3dBackBuffer.Get() != cameraBackBuffer.Get()) {
		...
		Kore::affirm(device->CreateRenderTargetView(m_d3dBackBuffer.Get(), nullptr, &m_d3dRenderTargetView));
		...
		Kore::affirm(device->CreateRenderTargetView(m_d3dBackBuffer.Get(), &leftRtvDesc, &m_d3dRenderTargetViewLeft));
		...
		Kore::affirm(device->CreateRenderTargetView(m_d3dBackBuffer.Get(), &rightRtvDesc, &m_d3dRenderTargetViewRight));
	...
	if (m_d3dDepthStencilView == nullptr) {
		...
		CD3D11_TEXTURE2D_DESC depthStencilDesc(DXGI_FORMAT_D16_UNORM, static_cast<UINT>(m_d3dRenderTargetSize.Width),
		...
		Kore::affirm(device->CreateTexture2D(&depthStencilDesc, nullptr, &depthStencil));
		...
		Kore::affirm(device->CreateDepthStencilView(depthStencil.Get(), &depthStencilViewDesc, &m_d3dDepthStencilView));
		...
		Kore::affirm(device->CreateDepthStencilView(depthStencil.Get(), &leftDSVDesc, &m_d3dDepthStencilViewLeft));
		...
		Kore::affirm(device->CreateDepthStencilView(depthStencil.Get(), &rightDSVDesc, &m_d3dDepthStencilViewRight));
	}
}
