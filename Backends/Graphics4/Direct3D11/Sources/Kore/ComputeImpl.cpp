...
ComputeShader::ComputeShader(void* _data, int length) {
	...
	Kinc_Microsoft_Affirm(device->CreateComputeShader(this->data, this->length, nullptr, (ID3D11ComputeShader**)&shader));

	Kinc_Microsoft_Affirm(device->CreateBuffer(&CD3D11_BUFFER_DESC(getMultipleOf16(constantsSize), D3D11_BIND_CONSTANT_BUFFER), nullptr, &constantBuffer));
}
