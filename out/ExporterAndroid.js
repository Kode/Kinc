...
class ExporterAndroid extends Exporter_1.Exporter {
    ...
    exportSolutionEclipse(solution, from, to, platform, vr) {
        ...
        if (Project_1.Project.koreDir.toString() != "")
            file = file.replace(/{Android-Backend-Sources}/g, path.resolve(Project_1.Project.koreDir, 'Backends', 'Android', 'Sources').replace(/\\/g, '/'));
        if (Project_1.Project.koreDir.toString() != "")
            file = file.replace(/{OpenGL-Backend-Sources}/g, path.resolve(Project_1.Project.koreDir, 'Backends', 'OpenGL2', 'Sources').replace(/\\/g, '/'));
        if (Project_1.Project.koreDir.toString() != "")
        ...
        for (let fileobject of project.getFiles()) {
            ...
            if (filename.endsWith(".c") || filename.endsWith(".cpp") || filename.endsWith(".cc") || filename.endsWith(".s"))
                files += path.relative(path.resolve(to, 'jni'), path.resolve(from, filename)).replace(/\\/g, '/') + " ";
        }
        this.p("LOCAL_SRC_FILES := " + files);
        let defines = "";
        for (let def of project.getDefines())
            defines += "-D" + def.replace(/\"/g, "\\\"") + " ";
        if (vr === 'gearvr') {
        ...
        let includes = "";
        for (let inc of project.getIncludeDirs())
            includes += "$(LOCAL_PATH)/" + path.relative(path.resolve(to, 'jni'), path.resolve(from, inc)).replace(/\\/g, '/') + " ";
        if (vr === 'gearvr') {
