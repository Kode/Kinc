...
namespace D3D12TranslationLayer {
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ ReferenceDataManager::ReferenceDataManager(VIDEO_DECODE_PROFILE_TYPE profileType)
	...
	_Use_decl_annotations_ UINT16 ReferenceDataManager::StoreFutureReference(UINT16 index, std::shared_ptr<VideoDecoderHeap> &decoderHeap,
		...
		if (remappedIndex == m_invalidIndex) {
			...
			if (index < referenceDatas.size() && referenceDatas[index].originalIndex == m_invalidIndex) {
				remappedIndex = index;
			}
		}
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void ReferenceDataManager::TransitionReferenceOnlyOutput(ID3D12Resource *&pOutputReferenceNoRef, UINT &OutputSubresource) {
