...
namespace D3D12TranslationLayer {
	CViewSubresourceSubset::CViewSubresourceIterator::CViewSubresourceIterator(CViewSubresourceSubset const &SubresourceSet, UINT16 ArraySlice,
	                                                                           UINT8 PlaneSlice)
	    : m_Subresources(SubresourceSet), m_CurrentArraySlice(ArraySlice), m_CurrentPlaneSlice(PlaneSlice) {}
	...
	UINT CViewSubresourceSubset::CViewSubresourceIterator::StartSubresource() const {
		return ComposeSubresourceIdxExtended(m_Subresources.m_BeginMip, m_CurrentArraySlice, m_CurrentPlaneSlice, m_Subresources.m_MipLevels,
		                                     m_Subresources.m_ArraySlices);
	}

	UINT CViewSubresourceSubset::CViewSubresourceIterator::EndSubresource() const {
		return ComposeSubresourceIdxExtended(m_Subresources.m_EndMip, m_CurrentArraySlice, m_CurrentPlaneSlice, m_Subresources.m_MipLevels,
		                                     m_Subresources.m_ArraySlices);
	}
	...
	ProfileBufferInfo VP9BufferInfo = {
	    3,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PicParams_VP9), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_Slice_VPx_Short),
	...
	ProfileBufferInfo VP8BufferInfo = {
	    3,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PicParams_VP8), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_Slice_VPx_Short),
	     VIDEO_DECODE_BUFFER_TYPE_BITSTREAM, 0},
	    {1, 2, 0, 0},
	};
	ProfileBufferInfo H264BufferInfo = {
	    4,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PicParams_H264), VIDEO_DECODE_BUFFER_TYPE_INVERSE_QUANTIZATION_MATRIX,
	     sizeof(DXVA_Qmatrix_H264), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_Slice_H264_Short), VIDEO_DECODE_BUFFER_TYPE_BITSTREAM, 0},
	    {1, 2, 1280, 720},
	};
	ProfileBufferInfo H264MVCBufferInfo = {
	    4,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PicParams_H264_MVC), VIDEO_DECODE_BUFFER_TYPE_INVERSE_QUANTIZATION_MATRIX,
	     sizeof(DXVA_Qmatrix_H264), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_Slice_H264_Short), VIDEO_DECODE_BUFFER_TYPE_BITSTREAM, 0},
	    {1, 2, 1280, 720},
	};
	ProfileBufferInfo HEVCBufferInfo = {
	    4,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PicParams_HEVC), VIDEO_DECODE_BUFFER_TYPE_INVERSE_QUANTIZATION_MATRIX,
	     sizeof(DXVA_Qmatrix_HEVC), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_Slice_HEVC_Short), VIDEO_DECODE_BUFFER_TYPE_BITSTREAM, 0},
	    {1, 2, 1280, 720},
	};

	ProfileBufferInfo MPEG2BufferInfo = {
	    4,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PictureParameters), VIDEO_DECODE_BUFFER_TYPE_INVERSE_QUANTIZATION_MATRIX,
	     sizeof(DXVA_QmatrixData), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_SliceInfo), VIDEO_DECODE_BUFFER_TYPE_BITSTREAM, 0},
	    {1, 1, 0, 0},
	};
	...
	ProfileBufferInfo MPEG4PT2BufferInfo = {
	    4,
	    {VIDEO_DECODE_BUFFER_TYPE_PICTURE_PARAMETERS, sizeof(DXVA_PicParams_MPEG4_PART2), VIDEO_DECODE_BUFFER_TYPE_INVERSE_QUANTIZATION_MATRIX,
	     sizeof(DXVA_QmatrixData), VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL, sizeof(DXVA_SliceInfo), VIDEO_DECODE_BUFFER_TYPE_BITSTREAM, 0},
	    {1, 1, 0, 0},
	};
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	VideoDecoder::VideoDecoder(ID3D12VideoDevice *pVideoDeviceNoRef, const D3D12_VIDEO_DECODER_DESC &desc) {
		pVideoDeviceNoRef->CreateVideoDecoder(&desc, IID_PPV_ARGS(&decoder));
	}

	//----------------------------------------------------------------------------------------------------------------------------------
	VideoDecoderHeap::VideoDecoderHeap(ID3D12VideoDevice *pVideoDeviceNoRef, const D3D12_VIDEO_DECODER_HEAP_DESC &desc) {
		pVideoDeviceNoRef->CreateVideoDecoderHeap(&desc, IID_PPV_ARGS(&heap));
	}

	extern "C" ID3D12Device *device;

	//----------------------------------------------------------------------------------------------------------------------------------
	VideoDecode::VideoDecode(VideoDecodeCreationArgs const &args)
	    : m_profileType(GetProfileType(args.Desc.DecodeProfile)), m_decodeFormat(args.Desc.DecodeFormat) {
		device->QueryInterface(&m_spVideoDevice);
		...
		m_spVideoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO_DECODE_SUPPORT, &decodeSupport, sizeof(decodeSupport));
		...
		m_decoderDesc.NodeMask = 0;
		m_decoderDesc.Configuration = decodeConfiguration;
		m_spVideoDecoder = std::make_unique<VideoDecoder>(m_spVideoDevice.get(), m_decoderDesc);

		m_decoderHeapDesc.NodeMask = 0;
	...
	void VideoDecode::ManageResolutionChange(const VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS *pOutputArguments) {
		...
		UINT height;
		UINT16 maxDPB;
		GetDecodeFrameInfo(&width, &height, &maxDPB);

		ID3D12Resource *pTextureArray = pOutputArguments->pOutputTexture2D;
		if (!pTextureArray) {
			assert(false);
		}
		...
		if (m_decodeFormat != outputResourceDesc.Format) {
			...
			m_spVideoDecoder = std::make_unique<VideoDecoder>(m_spVideoDevice.get(), decoderDesc);
			m_decoderDesc = decoderDesc;
		}

		if (!m_spCurrentDecoderHeap || m_decodeFormat != outputResourceDesc.Format || m_decoderHeapDesc.DecodeWidth != width ||
		    m_decoderHeapDesc.DecodeHeight != height || m_decoderHeapDesc.MaxDecodePictureBufferCount < maxDPB) {
			...
			m_spCurrentDecoderHeap = std::make_shared<VideoDecoderHeap>(m_spVideoDevice.get(), decoderHeapDesc);
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	template <typename T> static ETW_Pic_Entry LogCopyPicEntry(const T &src) {
		ETW_Pic_Entry dest;
	...
	void VideoDecode::ReleaseUnusedReferences() {
		// Method overview
		// 1. Clear the following m_referenceDataManager descriptors: textures, textureSubresources and decoder heap by calling
		...
		// 3. Call m_referenceDataManager.ReleaseUnusedReferences(); at the end of this method. Any references (and texture allocations associated) that were
		// left not marked as used in m_referenceDataManager by step (2) are lost.
		...
		case VIDEO_DECODE_PROFILE_TYPE_MPEG2: {
			...
			if ((pPicParams->wForwardRefPictureIndex != DXVA_INVALID_PICTURE_INDEX) || (pPicParams->wBackwardRefPictureIndex != DXVA_INVALID_PICTURE_INDEX)) {
				// References residency policy for frames that use at least one reference: Mark all references as unused and only mark again as used the ones
				// used by this frame
				m_referenceDataManager.ResetInternalTrackingReferenceUsage();
			}
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::PrepareForDecodeFrame(const VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *pInputArguments,
	                                                               const VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS *pOutputArguments) {
...

		{
			...
			UpdateCurrPic(pOutputArguments->pOutputTexture2D, pOutputArguments->SubresourceSubset.MinSubresource());
		}
		...
		case VIDEO_DECODE_PROFILE_TYPE_HEVC: {
			...
			//     When Index7Bits is used in the CurrPic and RefPicList members of the picture parameters structure, the value directly specifies the DXVA
			//     index of an uncompressed surface. When Index7Bits is 127 (0x7F), this indicates that it does not contain a valid index.
		...
		case VIDEO_DECODE_PROFILE_TYPE_H264: {
			// From H264 DXVA spec:
			// Index7Bits
			//     An index that identifies an uncompressed surface for the CurrPic or RefFrameList member of the picture parameters structure(section 4.0) or
			//     the RefPicList member of the slice control data structure(section 6.0) When Index7Bits is used in the CurrPic and RefFrameList members of the
			//     picture parameters structure, the value directly specifies the DXVA index of an uncompressed surface. When Index7Bits is used in the
			//     RefPicList member of the slice control data structure, the value identifies the surface indirectly, as an index into the RefFrameList array
			//     of the associated picture parameters structure.For more information, see section 6.2. In all cases, when Index7Bits does not contain a valid
		...
		case VIDEO_DECODE_PROFILE_TYPE_MPEG2: {
			...
			constexpr UINT postProcessingOptions =
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::DecodeFrame(const VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *pInputArguments,
	                                                     const VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS *pOutputArguments) {
		...
		d3d12InputArguments.CompressedBitstream.Offset =
		    pInputArguments->CompressedBitstream.Offset + pInputArguments->CompressedBitstream.pBuffer->GetSubresourcePlacement(0).Offset;
		d3d12InputArguments.CompressedBitstream.Size = pInputArguments->CompressedBitstream.Size;

		m_pParent->GetResourceStateManager().TransitionResource(pInputArguments->CompressedBitstream.pBuffer, D3D12_RESOURCE_STATE_VIDEO_DECODE_READ,
		...
		if (m_referenceDataManager.IsReferenceOnly()) {
		    ...
		    m_referenceDataManager.TransitionReferenceOnlyOutput(d3d12OutputArguments.ConversionArguments.pReferenceTexture2D,
		    ...
		    d3d12OutputArguments.ConversionArguments.DecodeColorSpace = CDXGIColorSpaceHelper::ConvertFromLegacyColorSpace(
		        !CD3D11FormatHelper::YUV(descReference.Format), CD3D11FormatHelper::GetBitsPerUnit(descReference.Format),false,
		        true, true);

		const D3D12_RESOURCE_DESC &descOutput = d3d12OutputArguments.pOutputTexture2D->GetDesc();
		d3d12OutputArguments.ConversionArguments.OutputColorSpace = CDXGIColorSpaceHelper::ConvertFromLegacyColorSpace(
		    !CD3D11FormatHelper::YUV(descOutput.Format), CD3D11FormatHelper::GetBitsPerUnit(descOutput.Format),   false, true,
		     StudioYUV=  true);
	...
	m_pParent->GetResourceStateManager().TransitionSubresources(pOutputArguments->pOutputTexture2D, pOutputArguments->SubresourceSubset,
		                                                        D3D12_RESOURCE_STATE_VIDEO_DECODE_WRITE, COMMAND_LIST_TYPE::VIDEO_DECODE);
	...
	for (UINT i = 0; i < _countof(d3d12OutputArguments.Histograms); i++) {
		if (pOutputArguments->Histograms[i].pBuffer) {
		    ...
		    d3d12OutputArguments.Histograms[i].Offset =
		        pOutputArguments->Histograms[i].Offset + pOutputArguments->Histograms[i].pBuffer->GetSubresourcePlacement(0).Offset;

		    m_pParent->GetResourceStateManager().TransitionResource(pOutputArguments->Histograms[i].pBuffer, D3D12_RESOURCE_STATE_VIDEO_DECODE_WRITE,
		                                                            COMMAND_LIST_TYPE::VIDEO_DECODE);
		}
		else {
	...
	m_pParent->GetVideoDecodeCommandList()->DecodeFrame1(m_spVideoDecoder->GetForUse(COMMAND_LIST_TYPE::VIDEO_DECODE), &d3d12OutputArguments,
		                                                 &d3d12InputArguments);

	UINT statusReportFeedbackNumber;
	DXVA_PicEntry CurrPic;
	UCHAR field_pic_flag;
	GetStatusReportFeedbackNumber(statusReportFeedbackNumber, CurrPic, field_pic_flag); // throw( _com_error )
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::CachePicParams(const VIDEO_DECODE_INPUT_STREAM_ARGUMENTS *pInputArguments) {
		UINT i = 0;
		...
		if (frameArgument.pData == nullptr || frameArgument.Size == 0) {
			// Invalid pic params.
			assert(false);
		}
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ HRESULT VideoDecode::GetDecodeFrameInfo(UINT *pWidth, UINT *pHeight, UINT16 *pMaxDPB) noexcept {
		HRESULT hr = S_OK;
		...
		*pMaxDPB = 0;
		...
		case VIDEO_DECODE_PROFILE_TYPE_MPEG2: {
			...
			*pMaxDPB = 2 + 1;
			...
			// but the display height used for the D3D12 decoder must be the full non-interlaced video height, not the interlaced sample height (half as the
			// video display size)
		...
		case VIDEO_DECODE_PROFILE_TYPE_H264: {
			...
			// wFrameWidthInMbsMinus1 Width of the frame containing this picture, in units of macroblocks, minus 1. (The width in macroblocks is
			// wFrameWidthInMbsMinus1 plus 1.) wFrameHeightInMbsMinus1 Height of the frame containing this picture, in units of macroblocks, minus 1. (The
			// height in macroblocks is wFrameHeightInMbsMinus1 plus 1.) When the picture is a field, the height of the frame is twice the height of the picture
			// and is an integer multiple of 2 in units of macroblocks.
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::UpdateCurrPic(ID3D12Resource *pTexture2D, UINT subresourceIndex) {
		switch (m_profileType) {
		case VIDEO_DECODE_PROFILE_TYPE_VC1:
		case VIDEO_DECODE_PROFILE_TYPE_MPEG2: {
			auto pPicParams = GetPicParams<DXVA_PictureParameters>();
			pPicParams->wDecodedPictureIndex =
			    m_referenceDataManager.StoreFutureReference(pPicParams->wDecodedPictureIndex, m_spCurrentDecoderHeap, pTexture2D, subresourceIndex);
		} break;
		...
		case VIDEO_DECODE_PROFILE_TYPE_H264: {
			...
			pPicParams->CurrPic.Index7Bits =
			    m_referenceDataManager.StoreFutureReference(pPicParams->CurrPic.Index7Bits, m_spCurrentDecoderHeap, pTexture2D, subresourceIndex);
		} break;
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	template <typename T> static void CopyNewStylePicParams(UINT &statusReportFeedbackNumber, DXVA_PicEntry &CurrPic, void *pParams) {
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::GetStatusReportFeedbackNumber(UINT &statusReportFeedbackNumber, DXVA_PicEntry &CurrPic,
	                                                                       UCHAR &field_pic_flag) noexcept {
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ VIDEO_DECODE_PROFILE_TYPE VideoDecode::GetProfileType(REFGUID DecodeProfile) noexcept {
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ VIDEO_DECODE_PROFILE_BIT_DEPTH VideoDecode::GetProfileBitDepth(REFGUID DecodeProfile) noexcept {
		ProfileInfo *pProfileInfo = GetProfileInfo(DecodeProfile);
	...
	GUID VideoDecode::GetDecodeProfile(VIDEO_DECODE_PROFILE_TYPE ProfileType, VIDEO_DECODE_PROFILE_BIT_DEPTH BitDepth) noexcept {
		...
		if (!m_DecodeProfilePerBitDepth[bitDepthIndex].has_value()) {
			...
			for (auto &profile : AvailableProfiles) {
				if (ProfileType == profile.DecodeProfileType && BitDepth == profile.DecodeProfileBitDepth) {
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ HRESULT VideoDecode::GetVideoDecoderBufferTypeCount(const VIDEO_DECODE_DESC *pDesc, UINT *pBufferTypeCount) noexcept {
		HRESULT hr = S_OK;
		*pBufferTypeCount = 0;
	...
	static bool IsXboxReuseDecoderProfileType(VIDEO_DECODE_PROFILE_TYPE ProfileType) {
		return ProfileType == VIDEO_DECODE_PROFILE_TYPE_H264 || ProfileType == VIDEO_DECODE_PROFILE_TYPE_H264_MVC ||
		       ProfileType == VIDEO_DECODE_PROFILE_TYPE_HEVC || ProfileType == VIDEO_DECODE_PROFILE_TYPE_VP9;
	}

	template <typename T> inline T Align(T uValue, T uAlign) {
		T uResult;
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::GetVideoDecoderBufferInfo(const VIDEO_DECODE_DESC *pDesc, UINT Index, VIDEO_DECODE_BUFFER_TYPE *pType, UINT *pSize,
	                                                                   bool IsXbox) {
		*pSize = 0;
			...
			*pSize = pProfileBufferInfo->Data[Index].BaseSize;
			...
			case VIDEO_DECODE_BUFFER_TYPE_SLICE_CONTROL: {
				...
				case VIDEO_DECODE_PROFILE_TYPE_H264:
					...
					// From spec ...satisfy the constraint that the number of slices in picture n is less than or equal to MaxMBPB * ( tr(n) - tr(n-1) ) /
					// SliceRate where MaxMBPS and SliceRate are the values specified in Tables A-1 and A-4... tr(n) = tr(n-1) = 1 / FPSRate = 1/300
					// MaxSliceNumber = 2319
			...
			case VIDEO_DECODE_BUFFER_TYPE_BITSTREAM: {
				if (IsXbox && IsXboxReuseDecoderProfileType(pProfileInfo->DecodeProfileType)) {
					...
					case VIDEO_DECODE_PROFILE_TYPE_H264_MVC:
						*pSize = 1800 * 1024;
						break;
					default:
						assert(pProfileInfo->DecodeProfileType == VIDEO_DECODE_PROFILE_TYPE_HEVC ||
						       pProfileInfo->DecodeProfileType == VIDEO_DECODE_PROFILE_TYPE_VP9);
						*pSize = 6912 * 1024;
						break;
					}
				}
				else {
					// *pSize = (Width * Height * 2 * pProfileBufferInfo->CompressedStreamMultiplier.Num) /
					// pProfileBufferInfo->CompressedStreamMultiplier.Denom; Smaller streams use a 1:1 compressed stream multiplier.  Memory optimization is not
					// as important at these smaller sizes.

					// Use a 1:1 ratio to calculate a minimum size against the MinWidth/MinHeight
					UINT minSize;
					UIntMult(pProfileBufferInfo->CompressedStreamMultiplier.MinWidth, pProfileBufferInfo->CompressedStreamMultiplier.MinHeight, &minSize);
					UIntMult(minSize, 2, &minSize);

					// Use the specified multipliers to calculate a size based on the stream width and height.
					UINT calcSize;
					UIntMult(Width, Height, &calcSize);
					UIntMult(calcSize, 2 * pProfileBufferInfo->CompressedStreamMultiplier.Num, &calcSize);
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::GetVideoDecoderSupport(ID3D12Device *pDevice12, UINT NodeIndex, const VIDEO_DECODE_DESC *pDesc,
	                                                                D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT &decodeSupport) {
		unique_comptr<ID3D12VideoDevice> spVideoDevice;
		pDevice12->QueryInterface(&spVideoDevice);
		...
		decodeSupport.BitRate = 0;
		spVideoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO_DECODE_SUPPORT, &decodeSupport, sizeof(decodeSupport));
	}
	...
	static bool SupportsArrayOfTexture(const D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT &decodeSupport, VIDEO_DECODE_PROFILE_TYPE ProfileType) {
		return ((decodeSupport.DecodeTier >= D3D12_VIDEO_DECODE_TIER_2 ||
		        ...
		        (ProfileType == VIDEO_DECODE_PROFILE_TYPE_H264 || ProfileType == VIDEO_DECODE_PROFILE_TYPE_H264_MVC ||
		         ProfileType == VIDEO_DECODE_PROFILE_TYPE_HEVC || ProfileType == VIDEO_DECODE_PROFILE_TYPE_VP9));
	}

	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::GetVideoDecoderConfigCount(ID3D12Device *pDevice12, UINT NodeIndex, const VIDEO_DECODE_DESC *pDesc,
	                                                                    UINT *pConfigCount) {
		UINT configCount = 0;
		D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT decodeSupport = {};
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ void VideoDecode::GetVideoDecoderConfig(ID3D12Device *pDevice12, UINT NodeIndex, const VIDEO_DECODE_DESC *pDesc, UINT configIndex,
	                                                               VIDEO_DECODE_CONFIG *pConfig, bool IsXbox) {
		...
		if (decodeSupport.SupportFlags & D3D12_VIDEO_DECODE_SUPPORT_FLAG_SUPPORTED) {
			if (supportedConfigIndex == configIndex) // found supported matching configIndex
			{
				...
				if (IsXbox && IsXboxReuseDecoderProfileType(ProfileType)) {
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	_Use_decl_annotations_ HRESULT VideoDecode::GetDecodingStatus(void *pData, UINT dataSize) noexcept {
		//**m_decodingStatus.ReadAvailableData(m_spVideoDecoder->GetForImmediateUse(), m_profileType, static_cast<BYTE *>(pData), dataSize); // throw(
		//_com_error )
