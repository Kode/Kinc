...
namespace D3D12TranslationLayer {
	...
	typedef enum {
		...
		VIDEO_DECODE_CONFIG_SPECIFIC_REUSE_DECODER = 1 << 15, // set by accelerator - This bit means that the decoder can be re-used with resolution change and
		                                                      // bit depth change (including profile GUID change from 8bit to 10bit and vice versa).
	...
	constexpr VIDEO_DECODE_PROFILE_BIT_DEPTH_INDEX GetIndex(VIDEO_DECODE_PROFILE_BIT_DEPTH BitDepth) {
		return static_cast<VIDEO_DECODE_PROFILE_BIT_DEPTH_INDEX>(BitDepth >> 1);
	}
	...
	public:
		...
		static void GetVideoDecoderBufferInfo(_In_ const VIDEO_DECODE_DESC *pDesc, _In_ UINT Index, _Out_ VIDEO_DECODE_BUFFER_TYPE *pType, _Out_ UINT *pSize,
		                                      bool IsXbox);
		...
		static void GetVideoDecoderConfig(_In_ ID3D12Device *pDevice12, UINT NodeIndex, _In_ const VIDEO_DECODE_DESC *pDesc, UINT configIndex,
		                                  _Out_ VIDEO_DECODE_CONFIG *pConfig, bool IsXbox);
		...
		bool IsArrayOfTexturesEnabled() const {
			return (m_ConfigDecoderSpecific & VIDEO_DECODE_CONFIG_SPECIFIC_ARRAY_OF_TEXTURES) == VIDEO_DECODE_CONFIG_SPECIFIC_ARRAY_OF_TEXTURES;
		}
	...
	protected:
		...
		void *GetPicParams() {
			return m_modifiablePicParams.get();
		}
		template <typename T> T *GetPicParams() {
			return static_cast<T *>(GetPicParams());
		}
		void *GetPicParams() const {
			return m_modifiablePicParams.get();
		}
		template <typename T> T *GetPicParams() const {
			return static_cast<T *>(GetPicParams());
		}
		...
		static void GetVideoDecoderSupport(_In_ ID3D12Device *pDevice12, UINT NodeIndex, _In_ const VIDEO_DECODE_DESC *pDesc,
		                                   _Out_ D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT &decodeSupport);
		...
		ReferenceDataManager m_referenceDataManager{m_profileType};
	...
	public:
	    BatchedVideoDecode(BatchedContext &Context, VideoDecodeCreationArgs const &Args) : BatchedDeviceChildImpl(Context, Args) {}
