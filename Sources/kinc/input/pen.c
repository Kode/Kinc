...
void (*kinc_eraser_press_callback)(int /*window*/, int /*x*/, int /*y*/, float /*pressure*/) = NULL;
void (*kinc_eraser_move_callback)(int /*window*/, int /*x*/, int /*y*/, float /*pressure*/) = NULL;
void (*kinc_eraser_release_callback)(int /*window*/, int /*x*/, int /*y*/, float /*pressure*/) = NULL;
...
void kinc_internal_eraser_trigger_press(int window, int x, int y, float pressure) {
	if (kinc_eraser_press_callback != NULL) {
		kinc_eraser_press_callback(window, x, y, pressure);
	}
}

void kinc_internal_eraser_trigger_move(int window, int x, int y, float pressure) {
	if (kinc_eraser_move_callback != NULL) {
		kinc_eraser_move_callback(window, x, y, pressure);
	}
}

void kinc_internal_eraser_trigger_release(int window, int x, int y, float pressure) {
	if (kinc_eraser_release_callback != NULL) {
		kinc_eraser_release_callback(window, x, y, pressure);
	}
}