...
namespace Kore {
	...
	float32x4 load(float a, float b, float c, float d) {
		float values[] = { a, b, c, d };
		return _mm_load_ps1(&values[0]);
	}

	float get(float32x4 t, int index) {
		return t.m128_f32[index];
	}

	float32x4 abs(float32x4 t) {
	...
	float32x4 neg(float32x4 t) {
		__m128 negative = _mm_set_ps1(-1.0f);
		return _mm_mul_ps(t, negative);
	}
	
	float32x4 reciprocalApproximation(float32x4 t) {
		return _mm_rcp_ps(t);
	}

	float32x4 reciprocalSqrtApproximation(float32x4 t) {
