...
class ExporterVisualStudio extends Exporter {
	...
	writeProjectDeclarations(project, solutionUuid) {
		...
		this.p("EndProject");
		for (let proj of project.getSubProjects()) writeProjectDeclarations(proj, solutionUuid);
	}
	...
	writeProjectBuilds(project, platform) {
		...
		for (let proj of project.getSubProjects()) writeProjectBuilds(proj, platform);
	}
...
//}
	...
	exportProject(from, to, project, platform, cmd, nokrafix) {
		for (let proj of project.getSubProjects()) exportProject(from, to, proj, platform, cmd, nokrafix);
		...
		this.p("<ItemGroup>", 1);
		for (let file of project.getFiles()) {
			if (file.file.endsWith(".h")) this.p("<ClInclude Include=\"" + from.resolve(file.file).toAbsolutePath().toString() + "\" />", 2);
		}
		this.p("</ItemGroup>", 1);
